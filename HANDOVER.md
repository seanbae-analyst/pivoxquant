...
> - **PR 전 리뷰(3갈래 병렬: 체크리스트+테스팅+유지보수+디자인 / 보안+마이그레이션+API+성능 / 적대적)**: CRITICAL 2건 수정 — ① `to_dict()` 의 `resolve_stock_name` 이 미등록 KR 종목에서 **라이브 KIS 호출**로 떨어짐 → 레지스트리 전용(`kr/us_stock_registry.get_name`)으로 교체 + 테스트로 고정 ② 커서 페이지네이션 정렬(created_at)과 커서(id) 불일치 → `id desc` 단독. 정보성 13건 수정(태그 LIKE 대소문자 SQLite/Postgres 불일치 → `func.lower`, 태그 `"`/`\` 거부, GET 라우트 rate limit, downgrade try/except 제거, journal 피드 오류 시 빈 상태 오표시 방지 + 재시도 스트립, 노트 200개 상한 명시, 글자수 code point 통일, 토큰 리터럴 정리, `.KS` 접미사 중복 칩, 데모 fixture 정규화, 카드 테스트, nightly purge 테스트). `# legal-exempt:` 마커 무의미 지적은 **오탐**(`tests/test_legal_deep_scan_local.py:129` 가 검사).
> - **브라우저 E2E(실 API, 2026-09-22)**: 로컬 백엔드 :5050 + `NEXT_PUBLIC_DEMO_MODE=0` 프론트, 비번 테스트 유저로 /journal 작성→카드→카운트, /pre-trade 종목 선택→30일 노트 되비침, /portfolio 종목 추가(추출한 TickerSearch)→행 액션→모달 작성까지 전부 실통과. **테스트로 못 잡은 버그 2건 발견·수정**: ① `API.observationNotes.create` 의 끝 슬래시 `/api/observation-notes/` 를 Next.js 가 308 로 리다이렉트(POST 유실, 브라우저는 https 로 따라가 SSL 오류) → 슬래시 제거(백엔드는 양쪽 등록 유지) ② 태그 입력이 keydown 이 아닌 경로(insertText/IME/blur)로 들어오면 "거래량," 처럼 쉼표째 저장 → commit 시 쉼표 분리. 로그인 원천 검사(`_LOGOUT_ALLOWED_ORIGINS` 는 :3000 하드코딩)는 로컬 임시 수정 후 원복. **Postgres 16 실검증**: 054 upgrade→downgrade→re-upgrade PASS, FK ondelete CASCADE 확인, 서비스 계층 커서·태그 필터 Postgres 에서 정상. 단 `test_migration_round_trip` 은 001 이 baseline 마커라 빈 DB 에서 002 에서 죽는다(pre-existing, 054 무관). brew `postgresql@16` 설치됨(서비스 미등록).
> - **검증(수정 후)**: pytest **2493 passed** · 법적 스위트 229 passed · vitest **528/528** · tsc·lint·build clean · regression guards passed · 054 upgrade/downgrade/재upgrade SQLite 실증(Postgres 는 로컬 없음 → CI/첫 배포에서 확인).
