...
class TestMaybeGenerateNameResolution:
    def _make_signal(self, ticker="124500.KQ", name=None):
        # Mimics the dict shape returned by services/quant/engine.py::analyze
        sig = {
            "ticker": ticker,
            "score": 75,
            "signal": "POSITIVE",
            "rec_shares": 10,
            "rec_investment": 1_000_000,
            "rec_timing": "Sized observation",
            "is_korean": True,
            "sell_pct": 0,
        }
        if name is not None:
    ...
    def test_kr_ticker_falls_back_to_resolver_when_name_missing(
        self, app, make_user, monkeypatch,
    ):
        """When the snapshot didn't carry a name (KR common case), the
        service must look it up via ``resolve_stock_name_with_db``."""
        from extensions import db
        from models import Alert
        from services import alert_service

        user = make_user(email="alert-kr@test.com")
        # Force the in-hours guard to allow the alert through regardless
        # of the wall clock — the dedup window is the only gate we want
        # exercised. We sidestep the ZoneInfo time check by patching
        # ``datetime.now`` inside alert_service.
        signal = self._make_signal(ticker="124500.KQ", name=None)

        with app.app_context(), \
             patch.object(
                 alert_service,
                 "datetime",
                 wraps=alert_service.datetime,
             ) as _dt, \
             patch(
                 "services.name_resolver.resolve_stock_name_with_db",
                 return_value="아이티센글로벌",
             ) as mock_resolve, \
             patch.object(alert_service, "safe_scrub", side_effect=lambda m, **kw: m):
            # Stub the market-hours check by short-circuiting the time
            # gate: rather than monkey-patching ZoneInfo, we patch
            # ``datetime.now`` to a known KST trading-hours moment.
            from datetime import datetime as _real_dt, timezone as _tz
            fake_now = _real_dt(2026, 5, 5, 1, 0, tzinfo=_tz.utc)  # 10:00 KST, Tue
            _dt.now.return_value = fake_now

            alert_service.maybe_generate(user["id"], signal)

            # Resolver was consulted because raw name was missing.
            assert mock_resolve.called, "name resolver must be invoked"

            row = (Alert.query
                   .filter_by(user_id=user["id"], ticker="124500.KQ")
                   .first())
            assert row is not None, "alert row must be persisted"
            # Stored message must carry the resolved name, NOT
            # "124500.KQ (124500.KQ)".
            assert "아이티센글로벌" in row.message
            assert "124500.KQ (124500.KQ)" not in row.message

    def test_resolver_skipped_when_snapshot_already_has_name(
        self, app, make_user,
    ):
        """If ``snapshot.name`` is real (not the ticker), no resolver
        call — preserves the original cheap path."""
        from extensions import db
        from models import Alert
        from services import alert_service

        user = make_user(email="alert-named@test.com")
        signal = self._make_signal(ticker="005930.KS", name="삼성전자")
        ...
        with app.app_context(), \
             ...
             patch.object(alert_service, "safe_scrub", side_effect=lambda m, **kw: m):
            from datetime import datetime as _real_dt, timezone as _tz
            _dt.now.return_value = _real_dt(2026, 5, 5, 1, 0, tzinfo=_tz.utc)

            alert_service.maybe_generate(user["id"], signal)

            # Resolver MUST NOT be invoked — the snapshot name was already real.
            assert not mock_resolve.called

            row = (Alert.query
                   .filter_by(user_id=user["id"], ticker="005930.KS")
                   .first())
            assert row is not None
            assert "삼성전자" in row.message

    def test_resolver_failure_falls_back_to_ticker(
        self, app, make_user,
    ):
        """Resolver returning None must NOT raise — the ticker fallback
        survives."""
        from extensions import db
        from models import Alert
        from services import alert_service

        user = make_user(email="alert-fail@test.com")
        signal = self._make_signal(ticker="999999.KQ", name=None)
        ...
        with app.app_context(), \
            ...
            row = (Alert.query
                   .filter_by(user_id=user["id"], ticker="999999.KQ")
                   .first())
            # Falls back to ticker — no crash, message contains ticker.
            assert row is not None
            assert "999999.KQ" in row.message
