...
> **⚠️ 2026-09-19 추기 — 이 문서의 추천(옵션2)은 반증됐다.**
> §35 의 "CEO 5분 액션"(15094808 라이선스 유형 직접 확인)을 2026-09-19 에 실행한 결과:
> - `data.go.kr/data/15094808/openapi.do` (페이지 수정일 2026-09-07) — **"이용허락범위 제4유형 : 출처표시 + 상업적 이용금지 + 변경금지"**, 그리고 **"본 데이터는 상업적 목적 여부와 상관 없이 제3자 무단 제공 및 재배포가 엄격히 금지됩니다. … 상업적 목적으로 이용할 경우 원천 소유자인 한국거래소의 KRX Data Marketplace 에서 유료로 구매하여야 합니다."** cc-zero 아님, 공공누리 2유형도 아님 — 더 엄격한 4유형.
> - 52주 고가/저가 필드 **없음**. 일별 종가로 직접 계산해야 하는데 그것이 "변경금지" 에 걸린다.
> - 갱신은 "영업일 하루 뒤 오후 1시 이후" — 금요일 종가는 월요일 13시. T+1 이 아니라 최대 T+3.
> - 같은 날 실측한 **KRX Open API**(openapi.krx.co.kr, 약관 시행 2025-12-26)도 제6조②(비상업적 목적으로만) · 제11조②(정보를 제3자에게 제공할 수 없다) 로 동일하게 막힘.
> - 미국 쪽(R8)도 무료 EOD 벤더 6곳(Alpha Vantage · Massive/Polygon · Tiingo · Finnhub · EODHD · Twelve Data) 전부 personal/non-commercial 한정. Nasdaq Data Link 는 무료 주가 피드 자체를 2018 년에 폐지.
>
> **따라서 무료·합법으로 시세를 유저에게 표시할 경로는 국내·미국 모두 없다.** 2026-09-19 CEO 결정: 벤더 시세 표시를 `MARKET_DATA_DISPLAY_ENABLED` / `NEXT_PUBLIC_MARKET_DATA_DISPLAY` 플래그(기본 꺼짐) 뒤로 넣고 `/portfolio` 는 취득가 기준으로, 52주 알림은 같은 플래그로 잠근다. FMP Data Display Agreement(R8, 접수됨)가 오면 미장부터 켠다. 조사 전문·출처 URL 은 메모리 `research_eod_data_licensing.md`. 아래 본문은 2026-06-09 시점 기록으로 보존한다.
