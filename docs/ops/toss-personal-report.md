...
| 진단 (`kind`) | 뜻 | 실현손익 |
|---|---|---|
| `share_count_changed` | 수량은 다른데 **취득원가 총액이 일치** — 병합·분할·합병 전환으로 주식 수만 다시 쓰였다. 들어오거나 나간 주식은 없다 | **그대로 신뢰 가능.** 단 그 이벤트를 사이에 둔 매도는 다른 눈금으로 계산됐을 수 있다 |
| `quantity` | 수량도 취득원가도 다름 — 이력 밖의 매수·입고 | 취득단가가 틀렸다 |
| `absent` | 보유 중인데 이력에 체결이 하나도 없음 | 〃 |
| `unmatched_sales` | 취득 기록 없이 판 주식 N주 (조회 시작일 이전 매수) | 그 몫은 **0 으로 빠져 있다** — 부풀리지 않는다 |
| `average` | 수량은 맞는데 평균단가가 다름 | 취득단가가 틀렸다 |
| `ghost` | 이력상 보유인데 계좌엔 없음 | 이력 밖 매도·출고 |

`share_count_changed` 만 있으면 리포트는 실현손익을 깎아내리지 않는다. 나머지가 하나라도 있으면
"부분"으로 표시하고 종목 이름을 붙인다. `--since` 를 넓혀 닫히는 것은 `absent`·`quantity`·
`unmatched_sales` 이고, `share_count_changed` 는 넓혀도 안 닫힌다 (Open API 에 그 이벤트가 없다).
억지로 맞추지 않는다.
