# PivoxQuant 인수인계서 — 과거 이력 (v55 이하)

> 2026-08-30 HANDOVER.md 에서 분리. 최신 이력은 루트 HANDOVER.md 참조.

## v55 2026-05-30 — Research 2라운드(행동심리+행동재무) + 정직한 컨셉 재포지셔닝

> **결론**: 변호사·컨설턴트 미팅 피드백 → 12 wave 전략 분석 → **deep research 2라운드 적대적 검증**(각 100+ agent). 핵심 발견: 문제(과잉거래)는 100% 진짜이나 **"기록이 투자를 개선한다"는 제품 핵심 가정은 2라운드 모두 입증 실패**. 컨셉을 "효능 약속 도구"에서 **"거래 전 거울(비판단·사전개입)"**로 재포지셔닝 권고. **코드 변경 0**(전략 검토). 상세: 메모리 `project_research_journaling.md` + `session_2026-05-29.md`.

### Research 결과 (1차 행동심리 + 2차 행동재무)

**✅ 확인 (강력한 근거)**
- 과잉거래가 수익 파괴: 고회전 11.4% vs 저회전 18.5% = **연 7%p 손실**. gross 수익 동일 → 종목 못 골라서가 아니라 거래비용 (Barber&Odean 2000, N=66,465)
- 처분효과: 오른 주식 2.65배 더 팔림. 판 winner가 든 loser보다 **연 3.4%p 우위**(홀딩이 실수)
- **🇰🇷 한국 개미 1,600% 회전율**(2020) = 글로벌 극단(US 250%/대만 300%/중국 500%). 자본시장연구원 20만명
- **비판단(non-judgmental) 프레이밍**이 전통 저널링 능가 (1차 RCT). 우리 "AI 점수화 폐기" 결정이 법적(PIPA§23/§6)뿐 아니라 심리적으로도 맞음

**🔴 반증/미입증 (제품 핵심 가정 흔들림)**
- **사적 거래일지(자유서술/구조화)가 편향감소/수익개선 = 확인 0건** (2라운드 모두)
- 유일 효과: 외부 경고가 처분효과 단기 감소 but **수익 개선 X**(한 맥락 오히려 악화), 3개월 소멸
- 🔴 choice-blindness: 사람은 자기 결정 이유를 **43% 지어냄(confabulate)** → "왜 샀어?" 기록 ≠ 진실
- "AI가 본다=장벽" 틀림: AI는 인간만큼 노출 유도, 장벽은 "**판단**"이지 "AI" 자체 아님
- "로컬/프라이빗=솔직함 극대화" 직접 반증
- 한국 문화 자기검열 / 자유서술vs구조화 / 앱 retention = **0건**(학술 답 없음 → A/B로만)

### 정직한 컨셉 재포지셔닝 (CEO 2026-05-30 승인)

| 버릴 것 | 가져갈 것 |
|--------|----------|
| "기록하면 투자 잘함" (🟥근거0·마케팅 금지) | "과잉거래가 당신 돈 까먹는다" (확실한 데이터) |
| 사후 일지 중심 | **거래 전 거울/마찰** (유일하게 효과 본 방향) |
| "왜 샀어"=진실 | 회고·자각 (진실 아님 인정) |
| 효능 약속 | 비판단 경험 + 신뢰 |

**컨셉 한 문장(권고)**: *"PivoxQuant는 당신을 부자로 만들어준다 약속하지 않는다. 당신이 과잉거래로 매년 돈을 잃고 있음을 거울처럼 보여주고, 거래 버튼을 누르기 직전 한 번 멈추게 한다. 채점·추천 없이."*

**무게중심 이동**: 사후 Artifact/기록 → **사전 개입(Pre-Trade)**. 기존 `services/pre_trade/` + Mistake Repeat Alarm + 과잉거래/회전율 가시화 대시보드(신규)가 research 근거 있는 방향. 사후 저널은 보조.

### 🟥 마케팅 카피 게이트 (research 기반, 출시 전 강제)
- **금지**: "투자 개선 / 수익 향상 / 더 나은 투자자 / 기록하면 잘함" (효능 미입증 = 거짓·표시광고법 위험)
- **허용**: "과잉거래로 연 7% 손실"(데이터 인용) / "거래 전 한 번 멈춤" / "채점하지 않는 거울"
- 효능 미입증은 카테고리 전체(Edgewonk/TraderSync도) → **효능 경쟁 말고 경험·신뢰·비판단으로**

### 다음 (검증 = A/B + 유저 인터뷰만)
- 핵심 가정("거래 전 거울/사전개입이 행동을 바꾸나")은 학술 답 없음 → **PivoxQuant 자체 A/B**(비판단 카피 vs 중립 / 사전개입 표시 vs 미표시)
- 실제 투자자 5명 인터뷰 (audit+research 공통 결론): "거래 전 거울, 쓸 거 같아?" — agent 불가, CEO 직접
