...
2-2. 이용자가 설정에서 발급한 가져오기 토큰으로, 이용자 본인이 운영하는 자동화 도구(예: 휴대폰 자동화 앱)가 전송한 체결 알림 텍스트의 파싱 (토큰 발급 시 별도 동의, 토큰은 대기 목록에 올리는 용도로만 사용 가능하며 이용자가 승인하기 전에는 기록되지 않음, 언제든 폐기 가능)
