...
|---|---|---|---|---|---|
| **Render Services, Inc.** | 미국 | 서비스 데이터 전반 (이메일·프로필·포트폴리오·거래내역 등 DB 저장 데이터) | 백엔드 호스팅 및 데이터 저장 | 서비스 이용 기간 | 필요 |
...
> 검토 비고 4(2026-06-05 규제 반영): 개인정보 보호법 개정(2026-02-12 국회 통과 / **2026-09-11 시행**)으로 국외이전 **영향평가제·사전심사제**가 신설됩니다. 위 7개 국외 수탁자에 대한 영향평가/사전심사 적용 여부는 변호사 확인이 필요합니다(자문 큐 Q6). ※ §28-8 국외이전 위반의 일반 과징금 상한은 전체 매출액의 100분의 3(3%) 이하이며, 동 개정의 "매출 최대 10%"는 중대 유출(고의·중과실 3년 내 반복 또는 1,000만 명 이상)에 대한 **별개의 징벌적 과징금 상한**입니다.
...
> 2026-09-02 정정: 본 절은 이전까지 "입력 내용 및 포트폴리오 요약이 Anthropic의
> Claude API로 전송됩니다" 라고 기재하고, 위 국외 이전 표에도 Anthropic, PBC 를
> **필수** 수탁자로 올려두고 있었습니다. 해당 기능(지원 챗봇 및 AI 분석)은
> 2026-09-01 서비스에서 제거되었고 관련 코드도 삭제되어, 그 시점 이후로는 실제
> 이전이 발생하지 않습니다. 일어나지 않는 이전에 대해 필수 동의를 받는 상태를
> 해소하기 위해 수탁자 목록과 본 절을 함께 정정했습니다.
>
> 「인공지능 발전과 신뢰 기반 조성 등에 관한 기본법」(2026-01-22 시행) 제31조의
> 고지 의무는 인공지능 기반으로 운용되는 서비스에 적용되므로, 현재 본 서비스는
...
- 서버는 국외 클라우드(Render 등) 데이터센터에서 운영되며 물리적 보안이 확보된 환경에서 관리됩니다.
