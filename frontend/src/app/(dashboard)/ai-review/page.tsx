    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="glass-surface rounded-xl p-4 spring-transition transition-all duration-300 hover:shadow-[0_4px_20px_rgba(0,0,0,0.3)]"
          >
            ...
            <p className="mt-2 text-lg font-bold font-mono text-white">{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-center">
        <Button
          ...
          disabled={loading}
          className="h-12 gap-2.5 bg-gradient-to-r from-cyan-500/10 to-emerald-500/10 text-cyan-400 border border-cyan-500/20 rounded-xl spring-transition px-8 text-sm font-semibold disabled:opacity-60"
        >
          {loading ? (
            <>
      ...
      {/* Error */}
      {error && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/[0.05] p-5 text-center text-sm text-red-400">
          {error}
        </div>
      )}
