...
export function TerminalNav({ user, onLogout, onTogglePanel, isPanelOpen }: Props) {
  ...
  return (
    <nav className="flex h-12 items-center glass-surface border-b border-[rgba(255,255,255,0.04)] px-4 shrink-0">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <button onClick={onTogglePanel}
          className={`hidden md:flex items-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-lg spring-transition transition-all duration-300 ${
            isPanelOpen
              ...
              : "text-zinc-600 hover:text-zinc-300 hover:bg-[rgba(255,255,255,0.04)]"
          }`}
          title="Toggle panel"
        >
        ...
        <button onClick={onLogout} className="text-[11px] text-zinc-600 hover:text-zinc-300 spring-transition transition-colors duration-300 hidden sm:block">
          Sign Out
        </button>
      </div>
    </nav>
