...
StockPilot은 단순한 주식 분석 도구가 아니다.
**사용자의 투자 성향을 분석**하고, 그에 최적화된 **퀀트 모델 + AI 어드바이저**를 배정하여
포트폴리오를 실시간으로 관리하고, 증권사 계좌와 직접 연동하여
**분석 → 판단 → 매매 → 리밸런싱**의 전 과정을 하나의 플랫폼에서 완결한다.

### 핵심 가치
1. **Adaptive Quant Engine** — 투자성향 분석 → 퀀트 모델 15+ 지표의 가중치/TP/SL/사이징이 자동 조정. 성향이 바뀌면 엔진이 따라감. 이게 StockPilot의 핵심.
...
### 기술 리스크
- API Rate Limit (yfinance, Alpaca, KIS) — 캐싱으로 완화
