    ...
    def send_email(self, user: User, pdf_bytes: Optional[bytes],
        ...
        if sg_key:
            try:
                ...
                if pdf_bytes:
                    ...
                    att = Attachment(
                        FileContent(enc),
                        FileName(f"portfolio_segment_{user.id}.pdf"),
                ...
                logger.error("SendGrid portfolio_segment send failed for user %s: %s",
                             user.id, exc)
                return False
        ...
        if smtp_host:
            try:
                ...
                logger.error("SMTP portfolio_segment send failed for user %s: %s",
                             user.id, exc)
                return False
