...
def _shareholder_letter(ctx_partial: dict[str, Any]) -> str:
    ...
    try:
        ...
        prompt = (
            "아래는 한 투자자의 연간 포트폴리오 기록이다. "
            "Warren Buffett의 주주 서한 톤(겸손·장기·절제)으로 2 문단(총 5-7문장, "
            "한국어)의 '연말 서한'을 작성하라.\n"
            "- 사실만 기술. 추천/매수/매도/조언/목표가 같은 단어 금지.\n"
            "- '귀하' 또는 '당신'으로 독자를 지칭.\n"
            "- 수치는 %만 인용 (금액 언급 금지).\n"
            "- 실수와 교훈을 정직하게 서술 (blame 없이).\n\n"
            f"연도: {year}\n"
            f"YTD: {_fmt_pct(ytd)} / 벤치마크: {_fmt_pct(bench)}\n"
            f"베스트 결정: {best_lines}\n"
            f"워스트 결정: {worst_lines}\n"
        )
