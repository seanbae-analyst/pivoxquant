    ...
    def send_email(self, user: User, pdf_bytes: Optional[bytes],
        ...
        if sg_key:
            try:
                ...
                if pdf_bytes:
                    ...
                    att = Attachment(
                        FileContent(enc),
                        FileName(f"finance_{month_label}.pdf"),
                ...
                logger.error("SendGrid monthly_finance send failed for user %s: %s",
                             user.id, exc)
                return False
        ...
        if smtp_host:
            try:
                ...
                logger.error("SMTP monthly_finance send failed for user %s: %s",
                             user.id, exc)
                return False
        return False
